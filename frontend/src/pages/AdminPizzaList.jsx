...
const AdminPizzaList = ({ searchQuery }) => {
  ...
  const handleDeletePizza = async (id) => {
    if (!window.confirm("Delete item permanently from catalog?")) return;
    try {
      const res = await fetch(`${API_URL}/pizzas/${id}`, {
        method: "DELETE",
  ...
  const validateForm = () => {
    ...
    if (
      !formData.smallPrice ||
      Number(formData.smallPrice) <= 0
    ) {
      newErrors.smallPrice =
        "Enter valid small price";
    }
    ...
    if (
      ...
      Number(formData.mediumPrice) <= 0
    ) {
      newErrors.mediumPrice =
        "Enter valid medium price";
    }
    ...
    if (
      !formData.largePrice ||
      Number(formData.largePrice) <= 0
    ) {
      newErrors.largePrice =
        "Enter valid large price";
    }
    ...
    try {
      ...
      payload.append(
        ...
        JSON.stringify([
          ...
          {
            size: "Medium",
            price: Number(formData.mediumPrice),
          },
          {
            size: "Large",
            price: Number(formData.largePrice),
          },
        ])
      );
      // CREATE OR UPDATE
      const url = editPizzaId
        ? `${API_URL}/pizzas/${editPizzaId}`
        : `${API_URL}/pizzas`;
      const method =
        editPizzaId ? "PUT" : "POST";
  ...
  // Live client-side processing loop powered by the parent query
    ...
    <div className="page-layout" >
      ...
      <div className="page-content">
        <div className="inventory-container">
            ...
            <tbody>
              {loading ? (
                ...
                filteredPizzas.map((pizza) => {
                  const mediumSize = pizza.sizes?.find(
                    (s) => s.size === "Medium"
                  );
                  ...
                  return (
                    <tr key={pizza._id}>
                      <td>
                        <div className="pizza-image-wrapper">
                          <img
                            src={pizza.imageUrl}
                            alt={pizza.name}
                            className={`table-pizza-img ${!pizza.isAvailable
                                ? "out-of-stock-img"
                                : ""
                              }`}
                            onError={(e) => {
                              e.target.src =
                                "https://placehold.co/50x50?text=Pizza";
                            }}
                          />

                          {!pizza.isAvailable && (
                            <span className="stock-overlay">
                              OUT OF STOCK
                            </span>
                          )}
                        </div>
                      ...
                      <td>
                        <span
                          className={`category-tag tag-${pizza.category}`}
                        >
          ...
          {/* CREATE RECORD MODAL SHEET OVERLAY */}
          {showModal && (
          ...
          {viewPizza && (
            ...
            <div className="modal-backdrop">
              ...
              <div className="view-modal-card">
                ...
                <img
                  src={viewPizza.imageUrl}
                  alt={viewPizza.name}
                  className="view-pizza-image"
                />
                ...
                <div className="view-details">
                  ...
                  <p className="view-description">
                    {viewPizza.description}
                  </p>

                  <div className="view-category">
                    {viewPizza.category.toUpperCase()}
                  </div>

                  <div className="view-status">
                    Status:
                    <span>
                      {viewPizza.isAvailable
                        ? " Available"
                        : " Out Of Stock"}
                    </span>
                  </div>
